...
class ParserModel(Model):
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE
        # W = tf.get_variable(initializer=xavier_weight_init,shape=(self.config.n_features*embed_size,self.config.hidden_size),dtype=tf.float32)
        # U = tf.get_variable(initializer=xavier_weight_init,shape=(self.config.hidden_size,self.config.n_classes),dtype=tf.float32)
        # b1 = tf.get_variable(initializer=tf.zeros_initializer,shape=self.config.hidden_size)
        # b2 = tf.get_variable(initializer=tf.zeros_initializer,shape=self.config.n_classes)

        h = tf.contrib.layers.fully_connected(x,num_outputs=self.config.hidden_size,activation_fn=tf.nn.relu,
            weights_initializer=xavier_weight_init(),biases_initializer=tf.zeros_initializer())
        h = tf.layers.dropout(h,rate=self.dropout_placeholder)
        pred = tf.contrib.layers.fully_connected(h,num_outputs=self.config.n_classes,activation_fn=tf.nn.relu,
            weights_initializer=xavier_weight_init(),biases_initializer=tf.zeros_initializer())
