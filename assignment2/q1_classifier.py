...
class SoftmaxModel(Model):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(dtype=tf.float32,
            shape=(self.config.batch_size,self.config.n_features),name='input')
        self.labels_placeholder = tf.placeholder(dtype=tf.float32,
            shape=(self.config.batch_size,self.config.n_classes),name='label')
        ### END YOUR CODE
    ...
    def add_prediction_op(self):
        ...
        """
        ### YOUR CODE HERE
        W = tf.get_variable('W',dtype=tf.float32,shape=(self.config.n_features,self.config.n_classes))
        pred = softmax(tf.matmul(self.input_placeholder,W))
        ### END YOUR CODE
        return pred
